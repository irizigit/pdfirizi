    ...
    try {
        ...
        if (userState.has(userId) && userState.get(userId).step.startsWith('add_lecture_')) {
            ...
            if (state.step === 'add_lecture_professor') {
                ...
                await message.reply(` ✅- المادة: ${state.subject} \n ✅-الفوج أو الافواج :${state.group}\n ✅-رقم المحاضرة :${state.number}\n ✅ إسم الاساد : ${state.professor }\n 📎 من فضلك أرسل ملف pdf \n💡 اكتب 'تراجع' لإلغاء العملية.` + signature);
                return;
            }
            
            if (state.step === 'add_lecture_file') {
                if (!message.hasMedia) {
                    await message.reply(`❌ لم ترسل ملفًا، يا ${senderName}📎 من فضلك أرسل ملف pdf \n💡 اكتب 'تراجع' لإلغاء العملية.` + signature);
                    return;
                }

                if (message.type !== 'document') {
                    await message.reply(`❌ لم ترسل مستندًا، يا ${senderName}! من فضلك أرسل  ملف PDF صالح\n💡 اكتب 'تراجع' لإلغاء العملية.` + signature);
                    return;
                }
                ...
                if (!isPdf) {
                    await message.reply(`❌ الملف ليس بصيغة PDF، يا ${senderName}! من فضلك أرسل ملف PDF صالح\n💡 اكتب 'تراجع' لإلغاء العملية.` + signature);
                    return;
                }

                if (!content.toLowerCase().endsWith('.pdf')) {
                    await message.reply(`، يا ${senderName}📎 من فضلك أرسل ملف pdf \n💡 اكتب 'تراجع' لإلغاء العملية.` + signature);
                    return;
                }
...
- اسم الملف: ${uniqueFilename}
 مشكور(ة) جزيل الشكر لمساهمتك القيمة، يا ${senderName}! ${signature}`;
        ...
        if (!isGroupMessage && userId === allowedUser) {
            ...
            if (content.toLowerCase() === 'pdf') {
                ...
                let lectureList = 'قم باختيار المحاضرة التي تريدها\n📚 قائمة المحاضرات:\n' ;
                lectures.forEach((lecture, index) => {
                    const title = metadata[lecture]?.name || lecture;
                    lectureList += `${index + 1}. ${title} (${metadata[lecture]?.subject || 'متاح'})\n`;
                });
                lectureList += `\n✉️ سيتم إرسال المحاضرة لك يا ${senderName} مباشرة في الخاص.\nأرسل كلمة تراجع لالغاء العملية\n`;
...
${signature}`;
            ...
            if (content.toLowerCase() === 'عرض المحاضرات' || content.toLowerCase() === 'pdf'|| content.toLowerCase() === 'Pdf'|| content.toLowerCase() === 'عرض المحاظرات') {
                const lectures = getLecturesList();
                if (lectures.length === 0) {
                    await message.reply(`📂 لا توجد محاضرات حالياً، يا ${senderName}.` + signature);
                    return;
                ...
                let lectureList = 'أرسل رقم المحاضرة التي تريد  \n📚 قائمة المحاضرات:\n' ;
                lectures.forEach((lecture, index) => {
                    const title = metadata[lecture]?.name || lecture;
                    lectureList += `${index + 1}. ${title} (${metadata[lecture]?.subject || 'متاح'})\n`;
                });
...
👨‍🏫 الأستاذ: ${lectureInfo.professor || '-'}\n
${senderName}تفضل هاهيا المحاضرة لي طلبتي يا 
\n${signature} `
